...
class ModalUrl extends Component {
  ...
  static propTypes = {
    ...
    closeType: PropTypes.oneOfType([
      PropTypes.func,
      PropTypes.oneOf(['back', 'replace'])
    ]),
  ...
  render () {
    ...
    const isOpen =
      open &&
      (typeof id === 'function' ? !!id(this.idFromUrl) : this.idFromUrl === id)
    const currentTitle =
      typeof title === 'function' ? title(this.idFromUrl) : title
    return children ? (
      <Modal
        {...props}
        open={isOpen}
  ...
  <Switch>
    <Route
      component={({ history, location }) => (
        <ModalUrl history={history} location={location} {...props} />
      )}
    />
  </Switch>
))
