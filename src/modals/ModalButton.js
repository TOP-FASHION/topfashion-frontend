...
export default withRouter(class ModalButton extends Component {
  static propTypes = {
  ...
  click = event => {
    this.props.onBeforeOpening(() => {
      ...
      if (result !== search) {
        const run = () => history.push(window.location.pathname + result + window.location.hash)
        if (delay) {
  ...
  render () {
    const {children, id, history, options, onClick, delay, ...props} = this.props
    return <Button {...props} onClick={this.click}>{children}</Button>
  }
})
