        ...
        <Group className="main-decorator__content">
          <Switch>
            ...
            <Route
              ...
              render={() => <BlogCategoryPage {...props} layout="grid" />}
              exact
            />
            <Route
              path="/promotions-news/:postId"
              render={() => <BlogPostPage {...props} layout="full" />}
              exact
            />
            <Route
              path="/account"
              component={MainDecorator.isLoggedIn(loginStore, AccountPage)}
            />
            <Route path="/contact-us" component={ContactUsPage} exact />
            {['about-us', 'policy'].map((page) => (
              <Route
                key={page}
                path={`/${page}`}
                component={() => <StaticPage page={page} />}
                exact
              />
            ))}
