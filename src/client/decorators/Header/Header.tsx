...
const Header = observer(({ layout = 'default' }: Props) => {
  ...
  const dropdownLogin = () => {
    ...
    const content = !loginStore.loggedIn ? (
      <Fragment>
        <div className="account-menu__form">
          ...
          <div className="account-menu__form-link">{tabsButtons}</div>
        </div>
      </Fragment>
    ) : (
      <Fragment>
            ...
            {userInfoStore.user ? (
              <Fragment>
                <div className="account-menu__user-name">
                  {userInfoStore.user.firstname} {userInfoStore.user.lastname}
                </div>
                <div className="account-menu__user-email">
                  {userInfoStore.user.email}
                </div>
              </Fragment>
            ) : null}
          </div>
    ...
    return <div className="account-menu">{content}</div>;
  };
        ...
        <div className="container-fluid">
          ...
          <div className="site-header__actions">
            ...
            <Indicator
              url="/wishlist"
              value={
                loginStore.loggedIn && wishlistGetProductsStore.productsWishlist
                  ? wishlistGetProductsStore.productsWishlist.length
                  : 0
              }
