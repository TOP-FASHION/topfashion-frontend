  ...
  const handleChangeQuantity = (item: any, quantity: any) => {
    setQuantities((state: any) => {
      const stateQuantity = state.quantities.find(
        (x: any) => x.itemId === item.product_id
      );
      if (!stateQuantity) {
    ...
    return productsCart
      ? Object.keys(productsCart).map((item) => {
          ...
          if (productsCart[item].product_image.length > 0) {
            image = (
              <Link to={`/category/product/${productsCart[item].product_id}`}>
                <img src={productsCart[item].product_image} alt="" />
              </Link>
            );
          }
          ...
          return (
            <tr key={productsCart[item].product_id} className="cart-table__row">
              ...
              <td className="cart-table__column cart-table__column--product">
                <Link
                  to={`/category/product/${productsCart[item].product_id}`}
              ...
              >
                <InputNumber
                  onChange={(quantity) =>
                    handleChangeQuantity(productsCart[item], quantity)
                  }
              ...
              <td className="cart-table__column cart-table__column--remove">
                <Button
                  variant="primary"
                  onClick={() =>
                    cartRemoveProductStore.removeProductCart(
                      productsCart[item].key
                    )
                  }
