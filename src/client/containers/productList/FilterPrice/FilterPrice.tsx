...
const FilterPrice = observer(({ from = undefined, to = undefined, min = 0, max = 100, step = 1, ...otherProps }: Props) => {
  const { currencyStore, productsStore, productsCategoriesStore } = React.useContext(AppContext)
  ...
  return (
    <div className='filter-price'>
      ...
      <div className='filter-price__title'>
        Price:
        {' '}
        <span className='filter-price__min-value'>{currencies(currency)} {fromLabel}</span>
        {' – '}
        <span className='filter-price__max-value'>{currencies(currency)} {toLabel}</span>
      </div>
    </div>
  )
})
