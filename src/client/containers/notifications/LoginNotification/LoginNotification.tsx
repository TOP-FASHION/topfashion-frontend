...
const LoginNotification = observer((props: any) => {
  ...
  const message = setMessages(props, messages, 'app.globalNotification.login.status.')
  ...
  const notify = (text: any) => {
    toast(message(statusTypes[loginStore.statusLogin], {
      error: text
    }), {
  ...
  return loginStore.statusLogin ? (
    <Fragment>
      {notify(loginStore.messageStatusLogin)}
    </Fragment>
  ) : null
})
