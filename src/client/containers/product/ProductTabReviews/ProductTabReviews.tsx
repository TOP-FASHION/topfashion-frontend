...
const ProductTabReviews = observer(({ product }: Props) => {
  const { productReviewsStore, productReviewsAddStore } = React.useContext(AppContext)
  const [page, setPage] = React.useState(1)
  // eslint-disable-next-line camelcase
  const { rating, review, reviewer, reviewer_email, onReviewerChange, onReviewerEmailChange, onReviewChange, onRatingChange } = productReviewsAddStore
  ...
  const currentPage = product.rating_count === 0 ? 1 : (
    Math.ceil(product.rating_count / 3)
  )

  const reviewsList = productReviews && productReviews.length > 0 ? productReviews.map((review: any, index: any) => (
        ...
        <div className='row'>
          <div className='col-12 col-lg-9 col-xl-8'>
            <Form noValidate>
              ...
              <Button onClick={() => productReviewsAddStore.addReview(product.id, currentPage)} className='btn btn-primary btn-lg'>
                Post Your Review
              </Button>
            </Form>
          </div>
