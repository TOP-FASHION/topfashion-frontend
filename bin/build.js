...
function buildLibrary() {
  return log(`Building application (./${buildFolder})`, (resolve, reject) => {
    runCmd(`webpack --config webpack/prod.client.js`, (err) => {
      if (err) {
        reject(err);
        return;
      }
      runCmd(`webpack --config webpack/prod.server.js`, (err) => {
        if (err) {
          reject(err);
          return;
        }
      ...
      const buildPackageJsonData = JSON.stringify(
        {
          ...
          scripts: {
            start:
              'cross-env PUBLIC_PATH=client/assets STATIC_PATH=server/static NODE_ENV=production node index.js',
          },
          dependencies,
        },
        null,
